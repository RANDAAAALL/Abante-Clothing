...
export async function actionEditUploadedProduct(
  ...
  try {
    ...
    // get files from FormData (they could be Files or empty strings)
    const frontFile = formData.get("product_item_image");
    const backFile = formData.get("product_item_back_image");

    // check if images should be removed
    // const removeFrontImage = formData.get("remove_front_image") === "true";
    // const removeBackImage = formData.get("remove_back_image") === "true";
    ...
    updateTag("weekend-offers-products");
