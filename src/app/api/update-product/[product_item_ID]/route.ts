...
export async function PUT(
  ...
  try {
    ...
    if (!existingProduct) {
      return NextResponse.json({ errorMessage: "Product not found. Failed to update" }, { status: 404 });
    }
    ...
    if (existingProductWithSameDetails) {
      return NextResponse.json(
        { errorMessage: "Can't update because a product with the same name, color, size, and type already exists" },
        { status: 400 }
      );
    }
    ...
    const hasChanges = 
      ...
      parseFloat(uploadFields.product_item_price) !== parseFloat(String(existingProduct.product_item_price)) ||
      (uploadFields.product_item_discount ? parseFloat(uploadFields.product_item_discount) : 0) !== existingProduct.product_item_discount ||
      ...
      uploadFields.product_item_material !== existingProduct.product_item_material ||
      uploadFields.product_item_construction !== existingProduct.product_item_construction ||
      uploadFields.product_item_design_features !== existingProduct.product_item_design_features ||
      parseInt(uploadFields.product_item_stock) !== existingProduct.product_item_stock ||
      uploadFields.product_item_status !== existingProduct.product_item_status ||
    ...
    if (!hasChanges) {
      return NextResponse.json({ errorMessage: "No changes detected - nothing to update" }, { status: 400 });
    }
    ...
    await prisma.product_items.update({
      ...
      data: {
        ...
        product_item_discount: uploadFields.product_item_discount ? parseFloat(uploadFields.product_item_discount) : 0,
