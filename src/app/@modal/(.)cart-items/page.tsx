            ...
            <div className="flex justify-between items-center mb-4">    
                ...
                {selectedItem.length >= 1 ? (
                    ...
                    <div className="mt-8 mx-auto text-sm text-center space-y-3.5">
                        <p className="font-bold"> Total: P{selectedItem.reduce((acc, item) => acc + (item.product.product_item_price ?? 0) * item.selectedSizeAndQty.qty, 0).toLocaleString("en-PH")}</p>
                        <button  onClick={() => router.replace('/login', { scroll: false })}
                        className="cursor-pointer bg-card-black-background text-white dark:bg-card-white-background dark:text-black rounded-sm py-3 px-7">Checkout</button>
                    </div>
                    </>
                ) : ( 
