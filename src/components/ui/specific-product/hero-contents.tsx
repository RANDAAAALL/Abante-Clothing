...
export default function HeroContents({
  ...
  // Memoized photo for performance
  const memoizedPhoto = useMemo(() => currentPhoto, [currentPhoto]);
  
  // ========== CLIENT-SIDE METADATA UPDATES ==========
  // This updates meta tags when color changes (for social sharing)
  useEffect(() => {
    ...
    updateMetaTag('og:title', `${baseTitle} (${colorName})`);
    updateMetaTag('og:description', currentPhoto.product_item_design_features || `Buy ${productName} in ${colorName} from Abante Clothing`);
    ...
    // Update Twitter tags
    updateTwitterTag('twitter:title', `${baseTitle} (${colorName})`);
    updateTwitterTag('twitter:description', currentPhoto.product_item_design_features || `Buy ${productName} in ${colorName} from Abante Clothing`);
